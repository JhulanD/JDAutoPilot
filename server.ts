...
app.post("/api/roi-report", async (req, res) => {
  try {
    ...
    try {
      ...
      console.warn("Local save failed (Normal on Vercel):", saveError);
    }
...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    ...
    // Check if dist exists, just in case
    if (fs_sync.existsSync(distPath)) {
      app.use(express.static(distPath));
      app.get('*', (req, res) => {
        res.sendFile(path.join(distPath, 'index.html'));
      });
    } else {
      // Fallback relative to __dirname for common deployment patterns
      const fallbackDistPath = path.join(__dirname, 'dist');
      app.use(express.static(fallbackDistPath));
      app.get('*', (req, res) => {
        const indexPath = path.join(fallbackDistPath, 'index.html');
        if (fs_sync.existsSync(indexPath)) {
          res.sendFile(indexPath);
        } else {
          res.status(404).send("Application files not found. Please ensure 'npm run build' was successful.");
        }
      });
    }
  }
